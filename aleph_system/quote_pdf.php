...
$intro = $set['pq_intro'] ?? 'We have the pleasure to submit to you our quotation with the following specification:';
...
<style>
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    table.qp th{text-align:left;padding:6px 12px;font-weight:700;border:1px solid #e6bfa0;font-size:13px;background:#f6d5b8;}
    table.qp td{padding:6px 12px;border:1px solid #ddd;font-size:13px;}
    table.qp td.price strong{font-weight:700;}
    .opt td{color:#333;}
    .terms{margin-top:26px;}
    .sign{margin-top:34px;text-align:right;}
    .sign .i{font-style:italic;}
    .foot{margin-top:44px;text-align:right;color:#333;font-size:12px;}
...
<body>
    <div class="lh">
        ...
        <div class="contact">
            ...
            <?php foreach (array_filter(array_map('trim', explode(',', $set['company_address'] ?? 'Mekalles - Eliane Building, P.O.Box: 147 Mansourieh El Metn, 1253 2020 Lebanon'))) as $al): ?><?= h($al) ?><br><?php endforeach; ?>
            <span style="display:inline-block;margin-top:7px;">R.C.B. <?= h($set['company_rcb'] ?? '18872') ?><br>TVA: <?= h($set['company_tva'] ?? '248620-601') ?></span>
        </div>
    </div>
    ...
    <div class="item">
        ...
        <table class="qp">
            ...
            <tbody>
                ...
                <tr><td><?= h($t['label']) ?></td><td class="price"><strong><?= $m($t['total_price']) ?> <?= h($cur) ?></strong> + VAT</td></tr>
                <?php endforeach; ?>
                <?php foreach ($opts as $o): ?>
                <tr class="opt"><td><?= h($o['label']) ?></td><td class="price"><strong>+ <?= $m($o['price']) ?> <?= h($cur) ?></strong> + VAT</td></tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    </div>
    ...
    <div class="terms"><strong>Payment terms:</strong> <?= h($terms) ?> Should you need further details, please do not hesitate to contact us.</div>
